...
class Invoice(db.Model):
    ...
    status = db.Column(db.String(20), default='Unpaid')  # Status (Unpaid, Paid, Cancelled, etc.)

    # Relationship back to Sale
    sale = db.relationship("Sale", backref=db.backref("invoices", lazy=True))

    def __repr__(self):
        return f"<Invoice {self.id} - Sale {self.sale_id} - {self.status}>"
...
def invoice():
    ...
    # Prepare a list of dicts for easier template rendering (optional)
    invoice_list = []
    for inv in invoices:
        invoice_list.append({
            'id': inv.id,
