...
def create_invoice():
    ...
    (s.id, f"{s.customer.name} - {s.product.name}") for s in sales
]
    ...
    return render_template('create_invoice.html', form=form, sales=sales)
# -------------------- Generate Invoice --------------------
@app.route('/create_invoice', methods=['GET','POST'])
def create_invoice():
    if 'user_id' not in session:
    ...
    form = InvoiceForm()
    sales = Sale.query.all()
    form.sale_id.choices = [(s.id, f"{s.customer.name} - {s.product.name}") for s in sales]

    # Convert sales to plain dicts for JS
    sales_data = [
        {
            'id': s.id,
...
def invoice():
    ...
    # For each invoice, you can access sale details via the relationship:
    # invoice.sale.customer.name, invoice.sale.product.name, etc.
