...
@app.route('/mark-paid/<int:create_invoice>', methods=['POST'])
def mark_paid(create_invoice):
    if 'user_id' not in session:
        return redirect(url_for('login'))
    invoice = Invoice.query.get_or_404(create_invoice)
    invoice.status = 'Paid'
    db.session.commit()
...
def invoice():
    ...
    invoice_list = []  # Make sure this is defined before the loop
    for inv in invoices:
        invoice_list.append({
            'id': inv.id,
            'sale_id': inv.sale_id,
            ...
            'total': inv.total,
            'date': inv.date,  # matches template
            'status': inv.status
        })
