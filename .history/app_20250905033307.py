...
from forms import LoginForm, ChangePasswordForm, UpdateAccountForm, CustomerForm, ProductForm, SaleForm, InvoiceForm
...
with app.app_context():
    ...
    if not User.query.filter_by(username='admin').first():
        admin = User(username='admin', password=generate_password_hash('admin12'))
...
def create_invoice():
    ...
    if form.validate_on_submit():
        existing = Invoice.query.filter_by(sale_id=form.sale_id.data).first()
        if existing:
            flash('Invoice already exists for this sale.', 'warning')
        else:
            new_invoice = Invoice(sale_id=form.sale_id.data, status=form.status.data)
            db.session.add(new_invoice)
            db.session.commit()
