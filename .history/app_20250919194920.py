...
class Invoice(db.Model):
    ...
    customer_name = db.Column(db.String(120), nullable=False)  # Customer
    product_name = db.Column(db.String(120), nullable=False)   # Product
    quantity = db.Column(db.Integer, nullable=False)           # Quantity
    total = db.Column(db.Float, nullable=False)                # Total ($)
    date = db.Column(db.DateTime, default=datetime.utcnow)     # Date
    status = db.Column(db.String(20), default='Unpaid')        # Status
    
    # Optional: relationship back to Sale
    sale = db.relationship("Sale", backref=db.backref("invoices", lazy=True))
    
    def __repr__(self):
        return f"<Invoice {self.id} - {self.customer_name} - {self.status}>
